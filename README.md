...
```sh
npm install --save-dev @hazymoon/jevlint typescript
```
...
| `mutants` | Recall probes for `jevlint recall` (helpers in `@hazymoon/jevlint/mutate`). |
| `prepare` | Deterministic edit of the text before it's sent (helpers in `@hazymoon/jevlint/prepare`). |
...
```ts
import { createJevlint, loadConfig } from '@hazymoon/jevlint';
...
- **Recall:** fixtures show a question can work; recall shows it works on your code. Each `mutants` entry turns real compliant code into a violation (for example, removing a timeout). `jevlint recall` judges a deterministic sample of mutated files and reports the share caught. Aim for 0.9 or higher before marking a rule `owned`.
  ...
  ```ts
  import { dropMethodCalls } from '@hazymoon/jevlint/mutate';
